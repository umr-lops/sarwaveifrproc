...
def generate_intermediate_product(
    ds, model, scaler, bins, predicted_variables, kept_variables
):
    """
    ...
    if "burst" in ds.coords:
        tiles_stacked = tiles.stack(
            all_tiles=["burst", "tile_line", "tile_sample"], k_phi=["phi_hf", "k_gp"]
        )
    else:
        tiles_stacked = tiles.stack(
            all_tiles=["tile_line", "tile_sample"], k_phi=["phi_hf", "k_gp"]
        )
...
def generate_product_on_land(ds, predicted_variables, kept_variables, bins):
    ...
    for v in predicted_variables:
        ...
        v_pdf = xr.DataArray(
            data=np.full(v_mid.shape + shape, np.nan), dims=(f"{v}_mid",) + dims
        ).rename(f"{v}_pdf")
        v_pdf.attrs = attributes["pdf"]
        data_to_merge.append(v_pdf.assign_coords({f"{v}_mid": v_mid}))

        v_mean = xr.DataArray(data=np.full(shape, np.nan), dims=dims).rename(
            f"{v}_mean"
        )
        v_mean.attrs = attributes["mean"]
        data_to_merge.append(v_mean)

        v_most_likely = xr.DataArray(data=np.full(shape, np.nan), dims=dims).rename(
            f"{v}_most_likely"
        )
...
def predict_variables(
    ...
    res = tuple(
        predictions[:, indices[i] : indices[i + 1]] for i in range(len(indices) - 1)
    )
...
def format_dataset(ds, predictions, predicted_variables, kept_variables, bins):
    ...
    for i in range(n):
        ...
        v_pdf = (
            xr.apply_ufunc(lambda x: special.softmax(x, axis=1), predictions[i])
            .rename(f"{v}_pdf")
            .assign_coords({f"{v}_mid": v_mid})
            .to_dataset()
        )
        ...
        v_most_likely = compute_values(v_pdf, v, get_most_likely).rename(
            f"{v}_most_likely"
        )
        v_most_likely.attrs = attributes["most_likely"]
        data_to_merge.append(v_most_likely)
...
def get_attributes(var):
    ...
    attributes = {
        "mid": {
            "long_name": f'central values of the bins used for discretizing the range of {var_spec["long_name"]} encountered during neural model training',
            "units": var_spec["units"],
        },
        "pdf": {
            "long name": f'{var_spec["long_name"]} discrete probability density function',
            "units": "probability",
        },
        "mean": {
            "long name": f'first-order moment of the {var_spec["long_name"]} discrete probability density function',
            "units": var_spec["units"],
        },
        "most_likely": {
            "long name": f'most likely value of {var_spec["long_name"]} given its discrete probability density function',
            "units": var_spec["units"],
        },
        "std": {
            "long name": f'square root of the second-order moment of the {var_spec["long_name"]} discrete probability density function',
            "units": var_spec["units"],
        },
    }
